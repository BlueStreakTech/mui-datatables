...
|**`customHeadCellButtonRender`**|function||Function that returns a string or React component. Used for creating a custom header button to a column. This method only affects the display in the table's header, other areas of the table (such as the View Columns popover), will use the column's label. `function(option : object) => string`&#124;` React Component`
